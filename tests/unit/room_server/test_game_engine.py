...
class TestPlayerActions(unittest.TestCase):
    ...
    def test_add_player_success(self):
        ...
        self.assertTrue(player_position in [(1, 1), (1, 9), (9, 1), (9, 9)])
...
class TestCombatLogic(unittest.TestCase):
    ...
    def setUp(self):
        ...
        Map Layout:
        #####
        #S+ #  <- (1,1) Spawn, (2,1) Breakable Wall, (4,4) Spawn
        #  S#
        #####
        """
        custom_map = "#####\n#S+ #\n#  S#\n#####"
    ...
    def test_explosion_destroys_breakable_wall(self):
        ...
        # Map Recap: #S+ #. Player at (1,1). Wall at (2,1).
        ...
        # Wall at (2,1) should now be an Empty tile
    ...
    def test_explosion_kills_player(self):
        ...
        # Force victim to (1,2), directly below the Bomber player
    ...
    def test_explosion_blocked_by_unbreakable(self):
        ...
        p1 = engine.add_player("Bomber")
        victim = engine.add_player("Victim")

        # Ensure positions
        p1.position.x, p1.position.y = 1, 1
        victim.position.x, victim.position.y = 3, 1

        engine.place_bomb("Bomber")
        engine.explode_bomb(engine.bombs[0])

        # Victim should be alive because the wall blocked it
        self.assertTrue(victim.is_alive)

    def test_explosion_blocked_by_breakable(self):
        """Test that explosion stops after destroying a breakable wall."""
        # Custom map: #S+S#
        custom_map = "#####\n#S+S#\n#####"
        with patch("builtins.open", new_callable=mock_open, read_data=custom_map):
            engine = GameEngine()
        ...
        # Victim should be alive because the breakable wall stopped the blast
        self.assertTrue(victim.is_alive)

        # Breakable wall should be gone
        self.assertEqual(engine.grid[1][2], TileType.EMPTY)
